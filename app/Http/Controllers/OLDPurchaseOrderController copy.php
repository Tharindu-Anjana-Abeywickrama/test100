...
class PurchaseOrderController extends Controller
{
    ...
    {
    // Step 1: Get the paginator
    $paginator = PurchaseOrder::with(['user', 'territory.region'])
        ->where('created_by', Auth::id())
        ->orderBy('date', 'desc')
        ->paginate(10);

    // Step 2: Transform only the items (collection inside the paginator)
    ...
    // Step 3: Return with pagination
    return Inertia::render('PurchaseOrders/ViewPurchaseOrder', [
        'initialOrders' => $paginator->items(), // transformed items
        'pagination' => [
    ...
    public function getPurchaseOrders(Request $request) { 
        ...
        $paginator = $query->orderBy('date', 'desc')->paginate(10);
        $paginator->getCollection()->transform(function ($order) {
            return [
                'id' => $order->id,
                'region' => $order->territory->region->code ?? '',
